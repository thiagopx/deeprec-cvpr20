...
for dataset in datasets:
    for vshift_prop, vshift_sib18, enabled in zip([0, 3], [0, 10], ['off', 'on']):
        for method, vshift, template_fname in zip(['Proposed', 'Paix\~ao-b'], [vshift_prop, vshift_sib18], [template_fname_prop, template_fname_sib18]):
            ...
            for run in results:
                doc = run['doc'].split('/')[-1]
                records.append([method, dataset, doc, enabled, len(run['solution']), run['prep_time'], run['pw_time'], run['opt_time']])


df = pd.DataFrame.from_records(records, columns=('method', 'dataset', 'doc', 'enabled', 'n', 'pro', 'pw', 'opt'))
print(df)
...
for enabled in ['off', 'on']:
    for method in ['Proposed', 'Paix\~ao-b']:
        matrix.append(summary.loc[(method, enabled)].values)

matrix = np.array(matrix)
print(matrix)
gray = [0.3] * 3
p1 = ax1.bar(x, matrix[: 2, 0], width, color=colors[0], linewidth=1, edgecolor=gray) # pro
p2 = ax1.bar(x, matrix[: 2, 1], width, bottom=matrix[: 2, 0], color=colors[1], edgecolor=gray) # pw
p3 = ax1.bar(x, matrix[: 2, 2], width, bottom=matrix[: 2, 0] + matrix[: 2, 1], color=colors[2], edgecolor=gray) # opt

p4 = ax2.bar(x, matrix[2 :, 0], width, color=colors[0], linewidth=1, edgecolor=gray) # pro
p5 = ax2.bar(x, matrix[2 :, 1], width, bottom=matrix[2 :, 0], color=colors[1], edgecolor=gray) # pw
p6 = ax2.bar(x, matrix[2 :, 2], width, bottom=matrix[2 :, 0] + matrix[2 :, 1], color=colors[2], edgecolor=gray) # opt

ax1.set_ylabel('time (sec.)')
# # plt.yticks(np.arange(0, 81, 10))
ax2.legend((p1[0], p2[0], p3[0]), ('pro', 'pw', 'opt'), title='\\textbf{stage}', fontsize=16, title_fontsize=16)
plt.draw()
y1_ticks = [0, 0.5, 1.0, 1.5]
y2_ticks = [0, 2.5, 5.0, 7.5, 10]
for ax, title, y_ticks in zip([ax1, ax2], ['vertical shift (off)', 'vertical shift (on)'], [y1_ticks, y2_ticks]):
    ax.xaxis.grid(False)
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(('Proposed', 'Paix\~ao-b'))
    ax.set_title(title)
